...
import { computeKeyedHash } from '../../src/hash/core';
...
describe('Test mac via blake3 test vectors', () => {
  it('keyed hash should work with blake 3 test vector', async () => {
    ...
    const mac = (await computeKeyedHash(256, key, [data])).digest();
  ...
  it('compute should work', async () => {
    ...
    const data = ['test', 'this', 'mac'];
    const mac = await computeMac(key, data);
