...
import { getRecipientsPublicKeys } from '../../src/email/keyService';
...
import { publicKeyToBase64 } from '../../src/email/converters';
...
describe('Test key service functions', async () => {
  ...
  it('should successfully return recipient public keys with valid parameters', async () => {
    ...
    const result = await getRecipientsPublicKeys([mockEmail], mockToken);
  ...
  it('should handle 401 unauthorized error', async () => {
    ...
    await expect(getRecipientsPublicKeys([mockEmail], mockToken)).rejects.toThrow(
      'Could not get recipients public keys',
    );
  });
