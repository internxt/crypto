...
describe("Test email crypto functions", () => {
  ...
  it("throws error if binaryToEmail fails", async () => {
    const bad_binary: Uint8Array = new Uint8Array([
      49, 34, 44, 34, 116, 101, 115, 116, 32, 114, 101, 99, 105, 112, 105, 101,
      110, 116, 32, 50, 34, 44, 34, 116, 101, 115, 116, 32, 114, 101, 99, 105,
      112, 105, 101, 110, 116, 32, 51, 34, 93, 44, 34,
    ]);
    expect(() => binaryToEmail(bad_binary)).toThrowError(
      /Cannot convert Uint8Array to email:/,
    );
  });
  ...
  it("throws error if emailToBinary fails", async () => {
    ...
    expect(() => emailToBinary(bad_email as any as Email)).toThrowError(
      /Cannot convert email to Uint8Array:/,
    );
  });
});
