...
import { EmailBody, PublicKeys, PublicKeysBase64, User, HybridEncKey, PwdProtectedKey } from '../../src/utils/types';
import {
  emailBodyToBinary,
  binaryToEmailBody,
  publicKeyToBase64,
...
describe('Test email crypto functions', () => {
  ...
  it('throws error if public key converter to base64 fails', async () => {
    ...
    await expect(publicKeyToBase64(bad_key)).rejects.toThrowError(/Cannot convert public key to base64 public key:/);
  });
  it('throws error if base64 to public key converter fails', async () => {
    ...
    const bad_key: PublicKeysBase64 = {
      user: alice,
      eccPublicKey: 'not valid key',
      kyberPublicKey: 'not valid key',
    };
    await expect(base64ToPublicKey(bad_key)).rejects.toThrowError(/Cannot convert base64 public key to public key:/);
  });
  ...
  it('throws error if pwd protected key to base64 convertion fails', async () => {
    ...
    expect(() => pwdProtectedKeyToBase64(bad_key as any)).toThrowError(/Cannot convert key to base64/);
  });

  it('throws error if base64 to pwd protected key convertion fails', async () => {
    const bad_key = 'bad key';
    expect(() => base64ToPwdProtectedKey(bad_key)).toThrowError(/Cannot convert base64 to key/);
  });
  ...
  it('throws error if enc hybrid key to base64 convertion fails', async () => {
    ...
    expect(() => encHybridKeyToBase64(bad_key as any)).toThrowError(/Cannot convert key to base64/);
  });

  it('throws error if base64 to enc hybrid key convertion fails', async () => {
    const bad_key = 'bad key';
    expect(() => base64ToEncHybridKey(bad_key)).toThrowError(/Cannot convert base64 to key/);
  });
});
