...
import { decryptEmailBody, encryptEmailBody, deriveDatabaseKey, deriveEmailDraftKey, encryptEmailBodyAndSubject, decryptEmailBodyAndSubject } from '../../src/email-crypto';
...
describe('Test email crypto functions', () => {
  ...
  it('should throw an error if decryption fails', async () => {
    ...
    await expect(decryptEmailBody(encEmailBody, badEncryptionKey, aux)).rejects.toThrow(
     EmailSymmetricDecryptionError,
    );

    const badAux = new Uint8Array([4, 5, 6, 7, 8]);
    await expect(decryptEmailBody(encEmailBody, encryptionKey, badAux)).rejects.toThrow(
      EmailSymmetricDecryptionError,
    );
  });
