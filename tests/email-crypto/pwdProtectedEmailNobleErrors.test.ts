...
vi.mock('@noble/hashes/utils.js', async () => {
  const actual = await vi.importActual<
    typeof import('@noble/hashes/utils.js')
...
vi.mock('@noble/ciphers/aes.js', async () => {
  const actual = await vi.importActual<
    typeof import('@noble/ciphers/aes.js')
...
describe('Test email crypto functions', () => {
  ...
  const sharedSecret = 'test shared secret';
   ...
   it('throws EmailSymmetricEncryptionError when symmetric encryption fails', async () => {
    ...
    await expect(createPwdProtectedEmailAndSubject(emailAndSubject, sharedSecret)).rejects.toThrow(EmailSymmetricEncryptionError);
  });
  ...
  it('throws EmailPasswordProtectError when key wrapping fails', async () => {
    ...
    await expect(createPwdProtectedEmailAndSubject(emailAndSubject, sharedSecret)).rejects.toThrow(EmailPasswordProtectError);
  });
